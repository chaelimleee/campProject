...
//	0425 song List<BoardVO> findByBoardContentContaining(@Param("boardContent") String boardContent, @Param("page")int page, @Param("limit") int limit);
	...
	//0507 추가 //0514 유사검색기능 보완(like ex.'촉촉한맛' 검색말고 '촉촉'으로 검색가능)
	...
	// 0514 song 원글 제목(댓글 제외)으로 검색된 레코드 수
	int countByboardTitleContainingAndBoardOrigin(String searchWord, int i);

	// 0514 song 원글 내용(댓글 제외)으로 검색된 레코드 수 
	int countByBoardContentContainingAndBoardOrigin(String searchWord, int i);

	// 0514 song 원글 작성자(댓글 제외)로 검색된 레코드 수
	int countByMemberNickContainingAndBoardOrigin(String searchWord, int i);
