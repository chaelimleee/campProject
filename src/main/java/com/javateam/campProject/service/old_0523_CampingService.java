...
@Service
@Slf4j
public class old_0523_CampingService {
...
//					  .forEach(x -> { log.info("{}", x); });
	...
	public List<UserResultVO2> predictCampWithNaver(UserRequestVO userRequestVO) {
		...
		List<CampEntity> legacyCampList = (List<CampEntity>) campRepo.findAll();
		List<CampImgsVO> legacyCampImgsList = (List<CampImgsVO>) campImgsRepo.findAll();
		...
		legacyCampList = legacyCampList.stream()
									   ...
									   .toList();
		// filter 메서드에서 null 값 점검 주의 !
		legacyCampImgsList= legacyCampImgsList.stream().toList();
		...
		for (CampEntity campEntity : legacyCampList) {
			...
			log.info("for문 진입");
			ImgsN ++;	
			CampImgsVO campImgs = (CampImgsVO) legacyCampImgsList;
			...
			userResult.setImgName1(campImgs.getImgName1());
			userResult.setImgName2(campImgs.getImgName2());
			userResult.setImgName3(campImgs.getImgName3());
...
//
//		List<String> campImgslist = new ArrayList<String>();
//		
//		campImgslist = campImgsRepo.findByCampImgs(id);
//
//		return campImgslist;
//	}
