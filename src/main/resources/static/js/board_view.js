...
// 댓글 목록 읽어오기
function getAllReplies(originalBoardId, reName) {
	
	axios.get(`/campProject/board/getRepliesAll.do?boardCode=${originalBoardId}`) 
				...
				replyData = `<div id="reply_${reply.boardCode}" class="border-bottom border-dark-1 bg-light w-100 ps-4">
								...
								<div class="d-flex flex-row py-2">
									...
									<div id="reply_actual_writer_${reply.boardCode}" class="d-flex align-items-center ms-1 mt-1">
										...
										<!-- 실제 댓글 작성자 파악을 위해 id 등록 -->	
										<button id="reply_writer_${reply.reName}" type="button" class="btn btn-outline-success position-relative">
								...
								<div class="my-1 d-flex flow row">
									...
									<div class="col-8 d-flex justify-content-end">
										...
										<a href="#" id="reply_update_btn_${reply.boardCode}" class="btn btn-sm btn-outline-success me-2 mb-1" 
										   onClick="updateReply(${originalBoardId}, ${reply.boardCode}, '${reName}')">수정</a>
							
										<a href="#" id="reply_delete_btn_${reply.boardCode}" class="btn btn-sm btn-outline-success mb-1 me-3"
										   onClick="deleteReply(${originalBoardId}, ${reply.boardCode}, '${reName}')">삭제</a>
...
// 댓글 작성 : 인자 변경
function writeReply(originalBoardId, reName) {
	...
	replyWriteBtn.addEventListener('click', function(e) { // 함수 내부에는 이와 같은 표현 권장	
		...
		if (reName == 'anonymousUser'){
			...
			// 현재 로그인 한 회원
			// let reName = '[[${#authentication.name}]]';
			 			
			console.log("댓글 작성자 : ", reName);
			...
			if (boardPass.value.trim() == '') {
				...
				boardPass.focus();
	 			...
	 			if (replyWriteForm.value.length > 100) {
	 				...
	 				// 전송			  
	 				axios.post('/campProject/board/replyWrite.do', 
	 					{
							boardOrigin : originalBoardId,
...
function updateReply(originalBoardId, replyBoardId, reName) {
	
	console.log("원글 아이디, 댓글 아이디, 글 작성자 : " + originalBoardId + "," + replyBoardId +"," + reName);
	...
	// 댓글 수정 내용 전송 버튼 생성 : 수정용 패쓰워드 입력란 포함 // 0503 pass제거
	...
	replySubmitBtn.onclick = () => {
		...
		if (replyUpdateForm.value.trim() == "") {
		...
		} else if (replyActualWriter != reName) { // 회원이 실제 댓글 작성자가 아니라면 차단 !
			...
			//let boardUpdatePass = document.getElementById(`board_pass_${replyBoardId}`);//0503
			
			console.log("수정할 댓글의 아이디 : ", replyBoardId);
			console.log("수정할 댓글의 '원글' 아이디 : ", originalBoardId);
			console.log("작성자 아이디 : ", reName);
			//console.log("수정할 댓글 패쓰워드 : ", boardUpdatePass.value);//0503
			...
			axios.post('/campProject/board/replyUpdate.do', 
				{
				    boardCode : replyBoardId,
					boardContent : replyUpdateForm.value,
					reName : reName,
					//boardPass : boardUpdatePass.value, //0503
					boardOrigin : originalBoardId //0503
				}
			)
			 .then(function(response) {
			...
			//} // if (boardUpdatePass.value.trim() == '') { .... //0503 제외
...
function deleteReply(originalBoardId, replyBoardId, reName) {
	...
	// 댓글 삭제 및 전송 버튼 생성 : 삭제용 패쓰워드 입력란 포함 //0503 패스워드 폼 삭제
	let replySubmitBtns = `<div id="reply_submit_btns_${replyBoardId}" class="d-flex justify-content-end my-2">
	...
	replySubmitBtn.onclick = () => {	
		...
		if (reName == 'anonymousUser'){
			...
			alert("댓글 작성을 위해서 로그인이 필요합니다.");
			location.href = "/campProject/login";
		}			
	
		// 댓글 폼점검 : 비어 있는지 여부 점검
		// 유의) 여기서는 "댓글 삭제" 버튼을 직접 이용하기 때문에 원글(boardCode) 버튼을 활용 
		//let boardDeletePass = document.getElementById(`board_pass_${replyBoardId}`); //0503
		
		if (replyActualWriter != reName) { // 회원이 실제 댓글 작성자가 아니라면 차단 !
			...
			axios.post(`/campProject/board/replyDelete.do`,
			{
				boardCode : replyBoardId, // 삭제할 댓글 아이디
				originalboardCode : originalBoardId, // 댓글 목록 갱신을 위한 원글 아이디
				//boardPass : boardDeletePass.value // 삭제할 댓글 패쓰워드 0503
...
function deleteBoard(boardCode, boardWriter) {
	...
	// console.log("boardCode : ", boardCode, ", reName : ", reName);
	...
	boardDeleteBtn.onclick = function() {
		...
		if (confirm("정말 삭제하시겠습니까?") == true) {
			...
			if (boardDeletePass.value.trim() == '') {
				...
				let str = `/campProject/board/deleteProc.do?boardCode=${boardCode}&memberEmail=${boardWriter}`;
